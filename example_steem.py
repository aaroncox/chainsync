...
from blocksync.adapters.steemv2test import SteemV2TestAdapter

# adapter = SteemV2TestAdapter(endpoints=['https://api.steemitstage.com/'], debug=True)
# adapter = SteemV2Adapter(endpoints=['http://18.220.215.254:8080/'])
adapter = SteemAdapter(endpoints=['https://api.steemit.com'], debug=False)
# adapter = SteemV2Adapter(endpoints=['https://api.steemitstage.com/'], debug=False)
